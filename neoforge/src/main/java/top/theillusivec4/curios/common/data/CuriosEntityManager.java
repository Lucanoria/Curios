      ...
      namespaces.forEach(
          namespace -> packResources.listResources(PackType.SERVER_DATA, namespace,
              ...
              (resourceLocation, inputStreamIoSupplier) -> {
                ...
                ResourceLocation rl = ResourceLocation.fromNamespaceAndPath(namespace, path.substring("curios/entities/".length(), path.length() - ".json".length()));
  ...
  private static Map<EntityType<?>, Map<String, ISlotType>> getSlotsForEntities(
    ...
    for (JsonElement jsonEntity : jsonEntities) {
      ...
      if (entity.startsWith("#")) {
        ...
        EntityType<?> type = BuiltInRegistries.ENTITY_TYPE.getOptional(ResourceLocation.parse(entity)).orElse(null);
